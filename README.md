...
- `llm_judge` - LLM-as-judge: малая модель оценивает, отвечает ли ответ агента на вопрос по существу (смысловая проверка поверх формальных). Применяется в выборочных кейсах; при недоступности LLM критерий не валит прогон;
