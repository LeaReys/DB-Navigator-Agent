...
AI-агент, который помогает backend-разработчику быстро ориентироваться в
базах данных MS SQL Server: находит нужные таблицы и поля, объясняет
структуру таблиц, генерирует безопасные read-only T-SQL запросы и
получает данные из БД.

Идея простая: вместо того чтобы вручную лазить по схеме незнакомой базы,
можно спросить у агента на обычном языке - "где хранится статус
должника?" или "покажи структуру таблицы debt" - и получить ответ с
опорой на реальную схему БД.

## 1. Концепция и пользователь

Основной пользователь - backend-разработчик, которому часто нужно быстро
понять:

- где в базе хранится нужная бизнес-информация;
- какая структура у конкретной таблицы;
- как написать безопасный SELECT-запрос;
- какое значение или статус лежит в БД по конкретному идентификатору.

Агент закрывает эти задачи и при этом не даёт навредить базе: он работает
только на чтение и блокирует любые попытки изменить данные.

**Внешние системы, с которыми работает агент:**

- **MS SQL Server** - основная БД, подключение через pyodbc в read-only режиме;
- **ChromaDB** - локальный векторный стор для RAG-поиска по схеме;
- **OpenRouter / Ollama** - провайдеры LLM (облачный и локальный варианты);
- **LangFuse** - observability: трейсинг запусков, метрики, расход токенов.

---

## 2. Быстрый старт через Docker

Запуск проекта целиком - БД, агента и веб-интерфейс - без локальной установки Python и ODBC-драйвера.
...
cp .env.example .env
#    впишисать ключи, токены и модели в .env
...
LangFuse подключается опционально: если в `.env` заданы ключи - пишутся
трейсы; если нет - агент просто работает без трейсинга, ничего не падает.

---

## 3. Архитектура агента

Агент построен на **LangGraph** и представлет граф, по которому проходит каждый
запрос. В зависимости от типа вопроса маршрут получается разным: где-то
достаточно RAG-поиска, где-то нужно сгенерировать и выполнить SQL. 

### State

Состояние (`agent/state.py`) -  общий "контейнер", который проходит
через все узлы. Каждый узел читает нужные поля и дописывает свои
результаты.

| Поле | Тип | Описание |
|------|-----|----------|
| `user_query` | str | Исходный вопрос пользователя |
| `classification` | ClassificationResult | Тип запроса (выход роутера-классификатора) |
...
Минимум три инструмента, и как минимум один из них ходит во внешнюю
систему (реальную БД), а не просто крутит текст:
...
В графе три точки ветвления - это то, что делает агента нелинейным, а не
обычным pipeline:
...
3. **Роутер 3** - после `execute_query`: исправить SQL (`fix_sql`) или
   форматировать ответ. Цикл `fix_sql -> execute_query` ограничен счётчиком
   `sql_retry_count` (до 2 попыток), чтобы не зациклиться.
...
| 2 | SQL содержит мутирующий оператор, но классификатор пропустил | Pydantic-валидатор + проверка в коннекторе |
...
| **Avg Latency** | 35.8 s |
| **P90 Latency** | 52.8 s |
| **Всего времени** | 659.4 s |

### По критериям (качество)

| Критерий | Accuracy | Кейсов |
|----------|----------|--------|
| Правильная классификация | 100% | 17/17 |
| Правильные инструменты | 94% | 16/17 |
| Безопасность SQL | 100% | 17/17 |
| Ожидаемые термины в ответе | 88% | 15/17 |

### По категориям

| Категория | Pass Rate | Кейсов | Avg Latency |
|-----------|-----------|--------|-------------|
| navigation | 100% | 3/3 | 27 s |
| schema | 67%  | 2/3 | 34.5 s |
| script | 67% | 2/3 | 100.9 s |
| data | 100% | 3/3 | 26.7 s |
| unsafe | 100% | 3/3 | 8.6 s |
| unknown | 100% | 2/2 | 13.6 s |
...
## 10. Безопасность (security-checklist)

Главная идея - не полагаться на одну проверку, а
...
## 11. Возможные расширения проекта

- [ ] (?) HITL - уточняющие вопросы у пользователя (напримерЖ если не удалось найти таблицу);
- [ ] Allowlist таблиц и БД;
- [ ] Доступы по ролям - ограничения какие таблицы/схемы/бд доступны, какие инструменты дсотупны;
- [ ] Поиск по конкретной БД/Схеме - выбор в UI элементов, по которым производится поискж
- [ ] Продолжение диалога (сохранение контекста в рамках сессии).
