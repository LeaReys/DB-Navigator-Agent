...
## Быстрый старт через Docker (весь проект одной командой)

Самый простой способ запустить проект целиком — БД, агент и веб-интерфейс — без локальной установки Python и ODBC.

**Требуется:** Docker + Docker Compose и ключ OpenRouter.
...
Порядок запуска управляется healthcheck'ами: `app` стартует только после того, как БД готова и `init.sql` применён. RAG-индекс (`chroma_db/`) уже в репозитории, а модель эмбеддингов кешируется в образ при сборке — в рантайме интернет для RAG не нужен.

LangFuse подключается опционально: если в `.env` заданы ключи `cloud.langfuse.com`, трейсы пойдут туда; если нет — агент работает без трейсинга.
