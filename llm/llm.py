"""
agent/llm.py — унифицированная обёртка над LLM провайдерами.
...
Почему флаг в env, а не в коде?
  Потому что это деплой-решение, а не бизнес-логика. Один и тот же
  код работает на ноутбуке разработчика с Ollama и на сервере
  с OpenRouter — без единого изменения в Python-файлах.

Оба провайдера реализуют BaseChatModel, поэтому:
  - with_structured_output() работает одинаково
  - nodes.py не знает какой провайдер используется
  - замена провайдера = смена одной строки в .env
"""
...
# = Алиасы для обратной совместимости с nodes.py =======
# Оставляем чтобы не переписывать старые вызовы

def get_small_llm() -> BaseChatModel:
    """Малая модель: классификация и форматирование."""
    return get_llm("small")


def get_large_llm() -> BaseChatModel:
    """Большая модель: генерация SQL."""
    return get_llm("large")
